...
const mapAppointment = (dbAppt: any) => {
  const base = {
    ...
    recurring_request_id: dbAppt.recurring_request_id != null ? dbAppt.recurring_request_id.toString() : null,
...
export async function createAppointment(appt: any) {
  ...
  const { data, error } = await supabase.from('appointments').insert({
    ...
    recurring_request_id: appt.recurring_request_id != null ? parseInt(appt.recurring_request_id) : null,
  }).select(`
    *,
    dental_vehicles(vehicle_name),
    services(service_name),
...
// Bulk-corrects the price of every appointment generated by one recurring
// request (regardless of status), so an admin can fix a whole series in one
// action instead of editing each month individually. Returns how many rows
// were touched — 0 means this request predates the recurring_request_id
// link (approved before that column existed) and those appointments need to
// be corrected one at a time in the Appointments tab instead.
export async function updateAppointmentsPriceByRequest(requestId: string, price: number) {
  const supabase = client();
  const { data, error } = await supabase
    .from('appointments')
    .update({ price })
    .eq('recurring_request_id', parseInt(requestId))
...
export async function getRecurringRequest(id: string) {
  const supabase = client();
  const { data, error } = await supabase.from('recurring_requests').select(RECURRING_REQUEST_SELECT).eq('request_id', parseInt(id)).maybeSingle();
  if (error) throw new Error(error.message);
